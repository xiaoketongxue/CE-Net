...
from networks import deeplab_xception, deeplab_resnet, BRU_net, U_Net
...
network_name = 'U-Net'
viz = Visualizer(env=network_name)
...
net = U_Net.UNet(n_channels=3, n_classes=12)
...
if resume_epoch != nEpochs:
    ...
    for epoch in range(resume_epoch, nEpochs):
        ...
        if useTest and epoch > 2 and epoch % 3 == 0:
        # if useTest and epoch > 5 and epoch % 5 == 0:
            ...
            for ii, sample_batched in enumerate(testloader):
                ...
                save_path = os.path.join('show_domo', network_name)
                if not os.path.exists(save_path):
                    os.mkdir(save_path)

                misc.imsave(os.path.join(save_path, str(ii) + '.png'), show_image)
            ...
            print('mean error is {}, and mean value is {}'.format(mean_error, np.mean(mean_error)))
            ...
            if mean_error_mean < 3.5:
            # Save the model
            #     min_mean_error = mean_error_mean
                viz.log(info=mean_error_mean, win='mean_error_mean')

                save_model_path = os.path.join(save_dir, network_name)
                if not os.path.exists(save_model_path):
                    os.mkdir(save_model_path)

                torch.save(net.state_dict(), os.path.join(save_model_path, 'best_models-' + str(epoch) + '.pth'))
                print("Save model at {}\n".format(os.path.join(save_model_path, 'best_models-' + str(epoch) + '.pth')))
